...
- [x] **QA.7** M0.7 gate: full suite + ruff + hermetic seams. ✅ **Full suite (sequential, venv): 226 passed / 3 skipped / 0 failed (229 collected; +3 review-fix tests); M0.7-affected files 106/106 pass** (after review-fix re-run). Hermetic seams measured: `test_gates.py` (7), QEI truncation/echo/multitoken (4) + saturate-1, corpus_count −1/1200/real-0/env-override (engine), saturation fields (adaptive/delta), `recon_distill` byte-identical default + `lexicon_json` provenance + invalid-input errors (MCP), qei tool-level surfacing, headless flags/exit (inception). Ruff clean for new code; CI ruff scope (`scripts/`) unchanged with the ~15 pre-existing findings extra (unchanged).
- [x] **QA.8** **Live smoke of the M0.7 seams** (2026-09-12, MCP module entrypoints, topic "Agentic workflows in scientific literature review", session `rec_b651e2a9…`): ✅ **semantic isolation** — probe cache key `v1/…/m/semantic/q/41bd…` (distinct mode segment), 25-doc capped pool, **graceful 429 degradation** (arxiv rate-limited; engine merged remaining providers without aborting); ✅ **topics layer** — 8 anchored topics incl. thin `n=1` frontiers (XAI, Meta-analysis and systematic reviews, AI in Healthcare, Topic Modeling) plus dense `n=4` clusters (Scientific Computing and Data Management 0.86); ✅ **QEI fired live at 0.50** (top-10 wrapped 5 prompt-echo terms: agentic/workflows/scientific/literature/review ⇒ `>0.3` gate detects an over-generic inquiry); ✅ **corpus saturation seam (T7.3)** — `recon_delta` probed the thin frontiers against the real OpenAlex corpus: XAI `corpus_total` 73,287 → **`dense`** (a saturated field, not a novel gap — the pool was just tangential), scarcity/label semantics operational on live counts. Cache root `.cache/inception_recon/` (gitignored). Remaining live hook, only if needed before PR: headless `inception --grounded --auto-select` on a topic (wizard-level emission; engine/MCP paths already proven live).
