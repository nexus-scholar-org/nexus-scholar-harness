...
| Query construction | `scholar-search-kit` | `Query(text=..., max_results=..., year_min=..., year_max=...)` (order/limits live in `Query` model) | Steps 2-5 |
...
| `src/scholar_harness/recon/distiller.py` | Micro-taxonomy, metrics/datasets terms, sub-school clustering, anchor-evidence emission | consumes `pools/<sha>_pool.json` (plain JSON, no ML deps) | Pure-Python term-frequency baseline first; LLM augmentation optional & gated |
...
| MCP agent tools | `uv run --directory tools/scholar-agent-kit scholar-agent` | 15 `nexus_*` tools (see `09_mcp_integration.md`) — all confirmed backed by real kits |
