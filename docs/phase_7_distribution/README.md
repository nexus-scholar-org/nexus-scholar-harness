...
- [x] **P7.9** PyPI publication of `nexus-scholar` metapackage (live via Trusted Publisher / OIDC in `publish.yml`; standalone kit packaging deferred until external modular consumers appear).

*Metapackage live on PyPI (<https://pypi.org/project/nexus-scholar/>); standalone kit publication deferred until external modular consumers appear.*