...
def test_cli_download_single_doi(mock_process_doi, tmp_path):
    mock_process_doi.return_value = DownloadResult(doi="10.1234/test", success=True, file_path=tmp_path / "test.pdf", was_oa=True)
    
    result = runner.invoke(app, ["--doi", "10.1234/test", "--output", str(tmp_path)])
...
def test_cli_download_from_file(mock_process_doi, tmp_path):
    ...
    mock_process_doi.side_effect = [
        DownloadResult(doi="10.1111/1", success=True, file_path=tmp_path / "1.pdf", was_oa=True),
        DownloadResult(doi="10.2222/2", success=False, error_message="Not Open Access", was_oa=False)
    ]
    
    result = runner.invoke(app, ["--input", str(input_file), "--output", str(tmp_path)])
