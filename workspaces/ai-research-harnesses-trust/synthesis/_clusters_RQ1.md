# Consensus clusters touching RQ1 (55 total)
...
## [C19] verdict=UNRESOLVED bucket=unresolved score=0.0 claims=3
### The framework is open-source and available on GitHub and PyPI, enabling reproducible installations and community contributions.
Supporting studies: SCI-000096, SCI-000127, SCI-000159
Member claims:
- (RQ1|NEUTRAL) The framework is open-source and available on GitHub and PyPI, enabling reproducible installations and community contributions. -- [SCI-000096#section-8-1-installation#FULLTEXT-054]
- (RQ1|NEUTRAL) Reproducibility is supported by releasing source code, step-by-step reproduction scripts, the preprocessed dataset, and the annotation platform code, plus a README with setup instructions. -- [SCI-000127#reproducibility-statement#FULLTEXT-264]
- (RQ1|NEUTRAL) The authors open-source all components — code, data, model checkpoints, datastores, and the SCHOLARQABENCH benchmark — to support reproducibility and accelerate future research. -- [SCI-000159#section-7-conclusion#FULLTEXT-441]

## [C24] verdict=UNRESOLVED bucket=unresolved score=0.0 claims=4
### Every API call is logged locally in structured JSON with model identity, token counts, latency, retry count, and initiating phase, enabling granular cost profiling.
Supporting studies: SCI-000099, SCI-000118, SCI-000186
Member claims:
- (RQ1|NEUTRAL) Every module in M-Reason logs token usage, runtime statistics, and inference metadata, which the authors describe as vital for monitoring computational costs with commercial APIs and for objectively comparing system performance against manual curation. -- [SCI-000099#3-4-design-principles-and-rationale#FULLTEXT-061]
- (RQ1|NEUTRAL) M-Reason provides complete traceability and auditability from source evidence to final conclusions, exposing an interactive interface with seven agent terminals plus downloadable logs, prompts, and intermediate outputs from each iteration to support transparency and reproducibility. -- [SCI-000099#4-m-reason-interface#FULLTEXT-063]
- (RQ1|NEUTRAL) Every API call is logged locally in structured JSON with model identity, token counts, latency, retry count, and initiating phase, enabling granular cost profiling. -- [SCI-000118#body#FULLTEXT-196]
- (RQ1|NEUTRAL) The EvoScientist experiment-execution stage is designed to yield verifiable outputs (logs and metrics) and to produce a structured execution report for each proposal. -- [SCI-000186#3-methodology-problem-formulation#FULLTEXT-501]
...
## [C4] verdict=PROVISIONAL bucket=provisional score=0.0 claims=1
### A microservices architecture using Docker containers ensures portability and reproducible performance across diverse computing environments.
Supporting studies: SCI-000082
Member claims:
- (RQ1|NEUTRAL) A microservices architecture using Docker containers ensures portability and reproducible performance across diverse computing environments. -- [SCI-000082#body#FULLTEXT-014]

## [C27] verdict=PROVISIONAL bucket=provisional score=1.0 claims=1
### HySemRAG implements an eight-stage ETL pipeline integrated with a multi-agent RAG framework rather than a single monolithic generation system.
Supporting studies: SCI-000100
Member claims:
- (RQ1|NEGATIVE) HySemRAG implements an eight-stage ETL pipeline integrated with a multi-agent RAG framework rather than a single monolithic generation system. -- [SCI-000100#body#FULLTEXT-073]

## [C44] verdict=PROVISIONAL bucket=provisional score=0.0 claims=1
### Bio-SIEVE uses QLoRA fine-tuning on the LLaMA-7b and Guanaco-7B models trained on the Instruct Cochrane train split.
Supporting studies: SCI-000111
Member claims:
- (RQ1|NEUTRAL) Bio-SIEVE uses QLoRA fine-tuning on the LLaMA-7b and Guanaco-7B models trained on the Instruct Cochrane train split. -- [SCI-000111#body#FULLTEXT-149]
...
## [C67] verdict=PROVISIONAL bucket=provisional score=0.0 claims=1
### All prompts and LLM outputs are publicly available on Zenodo for reproducibility purposes.
Supporting studies: SCI-000129
Member claims:
- (RQ1|NEUTRAL) All prompts and LLM outputs are publicly available on Zenodo for reproducibility purposes. -- [SCI-000129#data-availability-statement#FULLTEXT-285]
...
## [C73] verdict=PROVISIONAL bucket=provisional score=1.0 claims=2
### Brain Researcher returns an audit bundle linking the committed plan, tool calls, artifacts, evidence, and claim verdicts, mediated by a Model Context Protocol server that controls which actions a model may take.
Supporting studies: SCI-000140
Member claims:
- (RQ1|NEGATIVE) Brain Researcher defines six terminal claim states (accepted, qualified, revised, blocked, rejected, deferred) with explicit adjudication criteria, and routes unresolved decisions for human escalation rather than treating the verdict as a veto. -- [SCI-000140#supplementary-methods-overview-of-the-su#FULLTEXT-339]
- (RQ1|NEUTRAL) Brain Researcher returns an audit bundle linking the committed plan, tool calls, artifacts, evidence, and claim verdicts, mediated by a Model Context Protocol server that controls which actions a model may take. -- [SCI-000140#introduction#FULLTEXT-341]

## [C75] verdict=PROVISIONAL bucket=provisional score=0.0 claims=1
### The Prompt Library is a collection of system prompts tailored to specific research-life-cycle tasks, each carrying metadata about the addressed task and available in multiple variants that users can see and modify.
Supporting studies: SCI-000141
Member claims:
- (RQ1|NEUTRAL) The Prompt Library is a collection of system prompts tailored to specific research-life-cycle tasks, each carrying metadata about the addressed task and available in multiple variants that users can see and modify. -- [SCI-000141#4-framework-for-ai-assisted-research-4-1#FULLTEXT-355]

## [C81] verdict=PROVISIONAL bucket=provisional score=0.0 claims=1
### High-quality training data is generated synthetically using the inference-time pipeline, with a two-step data filtering process (pairwise-filtering and rubric-filtering) to remove hallucinations and repetitive writing.
Supporting studies: SCI-000159
Member claims:
- (RQ1|NEUTRAL) High-quality training data is generated synthetically using the inference-time pipeline, with a two-step data filtering process (pairwise-filtering and rubric-filtering) to remove hallucinations and repetitive writing. -- [SCI-000159#section-2-3-training-high-quality-synthe#FULLTEXT-437]

## [C82] verdict=PROVISIONAL bucket=provisional score=0.0 claims=1
### The OpenScholar-Datastore (OSDS) is built from 45 million papers using peS2o v3, producing 234 million passages — described as the largest open-sourced datastore for scientific literature.
Supporting studies: SCI-000159
Member claims:
- (RQ1|NEUTRAL) The OpenScholar-Datastore (OSDS) is built from 45 million papers using peS2o v3, producing 234 million passages — described as the largest open-sourced datastore for scientific literature. -- [SCI-000159#section-2-1-openscholar-retrieval-pipeli#FULLTEXT-440]
...
## [C92] verdict=PROVISIONAL bucket=provisional score=0.0 claims=1
### The conventional SLR process is characterized as time-consuming, labor-intensive, and susceptible to human bias, motivating the human-AI collaborative modular design.
Supporting studies: SCI-000182
Member claims:
- (RQ1|NEUTRAL) The conventional SLR process is characterized as time-consuming, labor-intensive, and susceptible to human bias, motivating the human-AI collaborative modular design. -- [SCI-000182#abstract#FULLTEXT-497]
