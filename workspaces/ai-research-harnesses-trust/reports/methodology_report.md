...
*Workspace:* `ai-research-harnesses-trust` · *Generated:* 2026-09-09 · *Append-only provenance:* `audit/journal.jsonl` (65 events at D3 close)
...
- Every pipeline step is recorded in `audit/journal.jsonl` (append-only, 65 events at D3 close, per-event `action`/`agent_or_tool`/`outputs`/`metrics`).
