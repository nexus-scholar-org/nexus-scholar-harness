...
| SCI-000708 | CNN | U-Net (NoRGN RGB) ; U-Net (SynthNoRGN) ; U-Net (TrueRGN) ; U-Net (SynthFakeRGN) ; U-Net (S | 0.7530 |  | Reference absolute multiclass mIoU 75.3% (U-Net SynthFakeRGN trained on orthophoto data, evaluated on frame data); headline contributions are relative gains (+1.1% mIoU on RGB frame stream, +1.4% on orthophoto vs RGB-only). Tables I/II full |
