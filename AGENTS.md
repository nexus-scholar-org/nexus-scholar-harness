...
- **Distribution metapackage (P7.2):** `packaging/nexus-scholar/` is the `nexus-scholar` metapackage (source-bundling wheel: harness + 8 kits via hatchling `force-include`, entrypoints `nexus-scholar`/`scholar-agent`). Its kit pins come ONLY from `plugins.json` via the deterministic codegen `scripts/generate_nexus_scholar_pins.py` — never hand-edit `nexus_scholar_pins.json`; CI enforces freshness with `--check`. Build with `uv build --wheel packaging/nexus-scholar` (plain `uv build` fails on the out-of-tree force-include). Since P7.7 (lazy-imports), both wheel entrypoints answer `--help` on a minimal-dep env; CI smoke is a hard gate. `nexus-scholar init <title>` (P7.3) scaffolds a portable workspace in any folder (reuses the inception wizard; skills symlinked from `NEXUS_SKILLS_SRC`/wheel bundle/repo with copy fallback; `.env.example` + `.mcp.json` shipped).
...
- Workspaces were emptied on 2026-09-11; the directory is scaffolded by `scholar-harness inception`. Prior workspace content lives in git history — restore from snapshot `72090e5` (see `docs/internal/COMMIT_SNAPSHOTS.md`).
